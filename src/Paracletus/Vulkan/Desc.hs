  ...
  where dslCreateInfo = createVk @VkDescriptorSetLayoutCreateInfo
          ...
          &* setListCountAndRef @"bindingCount" @"pBindings"
              ...
              , createVk @VkDescriptorSetLayoutBinding
                ...
                &* set @"descriptorCount" 1
                &* set @"stageFlags" VK_SHADER_STAGE_VERTEX_BIT
                &* set @"pImmutableSamplers" VK_NULL              , createVk @VkDescriptorSetLayoutBinding
                $  set @"binding" 3
                &* set @"descriptorType" VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER
                &* set @"descriptorCount" (fromIntegral nimages)
                &* set @"stageFlags" VK_SHADER_STAGE_FRAGMENT_BIT
...
prepareDescriptorSet ∷ VkDevice → VkDescriptorBufferInfo → VkDescriptorBufferInfo → [VkDescriptorImageInfo] → VkDescriptorSet → Int → Anamnesis ε σ ()
prepareDescriptorSet dev bufferInfo mbufferInfo imageInfo descriptorSet nimages = liftIO $ withVkArrayLen descriptorWrites $ \dwLen dwPtr → liftIO $ vkUpdateDescriptorSets dev dwLen dwPtr 0 VK_NULL
  where descriptorWrites =
          ...
          , createVk @VkWriteDescriptorSet
            ...
            &* set @"descriptorType" VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER
            &* set @"descriptorCount" 1
            &* setVkRef @"pBufferInfo" mbufferInfo
            &* set @"pImageInfo" VK_NULL
            &* set @"pTexelBufferView" VK_NULL
          , createVk @VkWriteDescriptorSet
            $  set @"sType" VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET
