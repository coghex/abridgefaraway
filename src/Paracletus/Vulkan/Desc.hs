...
prepareDescriptorSet dev bufferInfo dynBufInfo dynTexBufInfo imageInfo descriptorSet nimages = liftIO $ withVkArrayLen descriptorWrites $ \dwLen dwPtr → liftIO $ vkUpdateDescriptorSets dev dwLen dwPtr 0 VK_NULL
