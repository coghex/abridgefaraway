...
createFramebuffers ∷ VkDevice → VkRenderPass → SwapchainInfo → [VkImageView] → VkImageView → VkImageView → Anamnesis r e s [VkFramebuffer]
...
createCommandBuffers ∷ VkDevice → VkPipeline → VkCommandPool → VkRenderPass → VkPipelineLayout → SwapchainInfo → VkBuffer → (Word32, VkBuffer) → [VkFramebuffer] → [VkDescriptorSet] → Anamnesis r e s (Ptr VkCommandBuffer)
createCommandBuffers dev pipeline commandPool rpass pipelineLayout SwapchainInfo{swapExtent} vertexBuffer (nIndices, indexBuffer) fbs descriptorSets
  | buffersCount ← length fbs = do
      ...
      allocResource (const $ liftIO $ vkFreeCommandBuffers dev commandPool (fromIntegral buffersCount) cbsPtr) $ do
...
createFrameSemaphores ∷ VkDevice → Anamnesis r e s (Ptr VkSemaphore)
createFrameSemaphores dev = newArrayRes ⌫ replicateM _MAX_FRAMES_IN_FLIGHT (createSemaphore dev)

createFrameFences ∷ VkDevice → Anamnesis r e s (Ptr VkFence)
...
data RenderData = RenderData
  ...
  , memoryMutator      ∷ ∀ r e s. VkDeviceMemory → Anamnesis r e s () }

drawFrame ∷ RenderData → Anamnesis r e s Bool
