...
instance PrimBytes TransformationMatrix
...
updateTransMat ∷ (Float,Float,Float) → VkDevice → VkExtent2D → VkDeviceMemory → Anamnesis ε σ ()
updateTransMat cam device extent uniBuf = do
  uboPtr ← allocaPeek $ runVk ∘ vkMapMemory device uniBuf 0 (bSizeOf @TransformationMatrix undefined) VK_ZERO_FLAGS
  let mmodel = DF4
                (DF4 32 0 0 0)
                (DF4 0 32 0 0)
                (DF4 0 0 32 0)
                (DF4 0 0 0  1)
      mtexi = DF3 1 1 1
  poke (castPtr uboPtr) (scalar $ TransformationMatrix {..})
  liftIO $ vkUnmapMemory device uniBuf
  -- these commands are all backwards
  -- ortho near far w h
  where mview = translate3 (vec3 (unT 1 cam) (unT 2 cam) (unT 3 cam))
        unT ∷ Int → (Float,Float,Float) → Float
        unT 1 (x,_,_) = 3.6*x
        unT 2 (_,y,_) = 3.6*y
        unT 3 (_,_,z) = z
...
createTransMatBuffers ∷ VkPhysicalDevice → VkDevice → Int → Anamnesis ε σ [(VkDeviceMemory, VkBuffer)]
createTransMatBuffers pdev dev n = replicateM n $ createBuffer pdev dev (bSizeOf @TransformationMatrix undefined) VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT ⌄ VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)
...
transMatBufferInfo ∷ VkBuffer → Anamnesis ε σ VkDescriptorBufferInfo
transMatBufferInfo uniformBuffer = return $ createVk @VkDescriptorBufferInfo
  $  set @"buffer" uniformBuffer
  &* set @"offset" 0
  &* set @"range" (bSizeOf @TransformationMatrix undefined)
