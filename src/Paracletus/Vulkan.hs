...
verticesex ∷ DataFrame Vertex '[XN 3]
verticesex = XFrame $ square `appendDF` withPos (+ vec4 2 0 0 0) square `appendDF` withTC (indexAtlas 0 0 2 2) (withTC (+ vec3 0 0 1) (withPos (+ vec4 0 2 0 0) square))
...
indicesex ∷ DataFrame Word32 '[XN 3]
indicesex = atLeastThree $ fromList $ oneRectIndices ⧺ map (+4) oneRectIndices ⧺ map (+8) oneRectIndices
